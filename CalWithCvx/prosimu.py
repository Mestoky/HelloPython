...
class ProSimu:
    ...
    def chp_unit2(self, unit):
        ...
        self.constraints += [unit.p2 == unit.p3 / 0.8 + unit.h1 / 4.66]
        self.constraints += [unit.p3 / 0.5 >= unit.mmain * 0.15]
        self.constraints += [unit.p1 >= 0]
        self.constraints += [unit.p2 >= 0]
        self.constraints += [unit.p3 >= 0]
        self.constraints += [unit.h1 >= 0]
    ...
    def chp_unit3(self, unit):
        ...
        unit.cost = sum(map(unit.costfun, unit.mmain))
        self.totalcost += unit.cost
        self.constraints += [unit.p1 >= unit.mmain * 0.9, unit.p1 <= unit.mmain]
    ...
    def chp_unit4(self, unit):
        ...
        unit.pschedule = unit.p1 + unit.p2 + unit.p3
        unit.hschedule = unit.h1 + unit.h0
        unit.cost = sum(map(unit.costfun, unit.mmain))
        self.totalcost += unit.cost
        self.constraints += [unit.mmain == unit.p1]
        self.constraints += [unit.p1 == unit.p2]
    ...
    def plot_result(self):
        ...
        # 负荷
        plt.figure()
        plt.plot(list(range(1, len(self.pload) + 1)), self.pload, 'cx-', label=u"电负荷曲线")
        plt.plot(list(range(1, len(self.hload) + 1)), self.hload, 'mo-', label=u"热负荷曲线")
        ...
        for unit in self.wpunits:
            plt.figure()
            plt.plot(list(range(1, len(unit.pschedule.value) + 1)), unit.pschedule.value, 'cx-', label=u"出力曲线")
            plt.plot(list(range(1, len(unit.maxwp) + 1)), unit.maxwp, 'r--', label=u"最大出力曲线")
        ...
        for unit in self.pcunits:
            plt.figure()
            plt.plot(list(range(1, len(unit.pschedule.value) + 1)), unit.pschedule.value, 'cx-', label=u"电出力曲线")
            plt.plot(list(range(1, len(unit.pschedule.value) + 1)), [unit.pbound[0]] * len(unit.pschedule.value), 'r--',
                     label=u"电出力下限")
            plt.ylabel(u"出力曲线(MW)")
            plt.legend()
            plt.title(u"%s出力曲线(MW)" % unit.name)
        ...
        for unit in self.chpunits:
            ...
            plt.plot(list(range(1, len(unit.pschedule.value) + 1)), [unit.pbound[0]] * len(unit.pschedule.value), 'r--',
                     label=u"电出力下限")
            plt.plot(list(range(1, len(unit.hschedule.value) + 1)), unit.hschedule.value, 'mo-', label=u"热出力曲线")
            plt.ylabel(u"出力曲线(MW)")
            plt.legend()
            plt.title(u"%s出力曲线(MW)" % unit.name)
            plt.show()
            if unit.rtype == 4:
                plt.figure()
                plt.plot(list(range(1, len(unit.H.value) + 1)), unit.H.value, 'mo-', label=u"储热量曲线")
